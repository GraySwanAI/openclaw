...
describe("openclaw-tools: subagents (sessions_spawn lifecycle)", () => {
  ...
  it("sessions_spawn runs cleanup flow after subagent completion", async () => {
    ...
    // Cleanup should patch child session metadata
    expect(patchParams.key).toBe(childSessionKey);

    // Subagent spawn call should be present.
  ...
  it("sessions_spawn reports timed out when agent.wait returns timeout", async () => {
    ...
    expect(calls.some((call) => call.method === "agent.wait")).toBe(true);
  });
  ...
  it("sessions_spawn announces with requester accountId", async () => {
    ...
    const agentCalls = calls.filter((call) => call.method === "agent");
    expect(agentCalls.length).toBeGreaterThanOrEqual(1);
    const announceParams = agentCalls
      .map(
        (call) =>
          call.params as
            | { lane?: string; accountId?: string; channel?: string; deliver?: boolean }
            | undefined,
      )
